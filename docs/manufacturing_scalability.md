...
## Digital Twin Simulation of Synthesis Processes

To accelerate the discovery and optimization of room-temperature superconductor synthesis, we introduce a **Digital Twin Simulation** module that models phase diagrams and reaction kinetics for the top candidate materials (H₃S, LaH₁₀, C-S-H, YH₁₀, CaH₁₂). This module is implemented as a Python class `DigitalTwinSimulator` in `run_pipeline.py` and integrates with the existing Monte Carlo cost/yield framework and the active learning loop.

### Phase Diagram Simulation
- Uses CALPHAD (CALculation of PHAse Diagrams) methodology combined with density functional theory (DFT) data from `dft_calculator.py` to construct pressure-temperature-composition phase diagrams for each candidate.
- Employs a Gibbs free energy minimization routine (via `thermopy` or custom Newton-Raphson solver) to identify stable phases at given (P, T, x) conditions.
- Outputs the stability region of the superconducting phase, including the critical pressure and temperature boundaries.
- For each candidate, the phase diagram is precomputed and cached in `data/phase_diagrams/` as JSON files, with fields: `phases`, `boundaries`, `critical_points`.

### Reaction Kinetics Simulation
- Models the synthesis reaction as a set of coupled ordinary differential equations (ODEs) representing precursor decomposition, hydrogen diffusion, and product formation.
- Uses Arrhenius kinetics with activation energies derived from DFT transition-state calculations or literature values.
- Simulates the time evolution of reactant and product concentrations under user-defined temperature and pressure ramps.
- Provides outputs: conversion rate, yield, and time to reach 90% completion.
- The ODE solver (SciPy `solve_ivp`) is wrapped in a `ReactionKinetics` class that accepts synthesis parameters (temperature profile, pressure profile, initial composition) and returns a `KineticsResult` object.

### Integration with run_pipeline.py
- The `DigitalTwinSimulator` is instantiated in the main pipeline loop after candidate selection (from the active learning or Bayesian optimization step).
- For each candidate, the simulator runs phase diagram and kinetics simulations using the current best-guess synthesis parameters (temperature, pressure, doping).
- Results (e.g., predicted yield, optimal temperature/pressure window) are fed back into the Bayesian optimizer to refine the next set of synthesis conditions.
- The simulator also writes a summary to `docs/simulation_reports/` for human review.
- Example usage in `run_pipeline.py`:
  ```python
  from digital_twin import DigitalTwinSimulator
  sim = DigitalTwinSimulator(candidate="H₃S")
  result = sim.run(temperature=2000, pressure=155, composition={"H":3,"S":1})
  print(f"Predicted yield: {result.yield:.2f}%")
  ```

### Data Flow
1. `candidate_materials.md` provides the list of candidates with predicted Tc and synthesis routes.
2. `dft_calculator.py` supplies DFT-derived thermodynamic and kinetic parameters.
3. The Monte Carlo module (described above) provides cost/yield distributions.
4. The Digital Twin Simulator combines these to produce a detailed process simulation.
5. The Bayesian optimizer uses the simulation output to propose improved synthesis parameters.
6. The active learning loop selects the next candidate for experimental validation.

This digital twin approach reduces the number of expensive experimental trials by pre-screening synthesis conditions in silico, accelerating the path to a manufacturable room-temperature superconductor.
