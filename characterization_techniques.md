...
## 12. Synchrotron X-ray Diffraction and Resistivity Measurements Under Pressure
- **Combined in situ measurements**: Synchrotron X-ray diffraction (XRD) and four-probe resistivity can be performed simultaneously in a diamond anvil cell (DAC) to correlate structural changes with the superconducting transition. The X-ray beam probes the crystal structure (lattice parameters, phase transitions) while electrical contacts measure resistivity as a function of temperature and pressure.
- **Experimental setup**: Use a DAC with a beryllium or cubic boron nitride gasket that is X-ray transparent. Micro-fabricated electrodes (e.g., Pt or Au) are deposited on the diamond anvil or gasket. A constant current source and nanovoltmeter measure the four-probe resistance. The X-ray diffraction pattern is collected using angle-dispersive geometry with a 2D detector (e.g., MAR345 or Pilatus). The sample is loaded with a pressure-transmitting medium (e.g., neon or helium) and a ruby chip for pressure calibration.
- **Data analysis**: The resistivity drop to zero at Tc is identified. Simultaneously, the XRD pattern is refined (e.g., using GSAS or DIOPTAS) to extract lattice parameters. A structural phase transition or a change in compressibility at Tc can indicate coupling between lattice and superconductivity. For hydrides, the appearance of a predicted high-symmetry phase (e.g., cubic Im-3m for H3S) is correlated with the onset of superconductivity.
- **Advantages**: Direct correlation of structure and transport eliminates sample-to-sample variability. Time-resolved measurements can capture metastable phases. Synchrotron sources provide high flux and small beam sizes (down to a few microns), enabling measurements on sub-millimeter samples.
- **Limitations**: The DAC environment introduces strain and pressure gradients. The X-ray beam may cause local heating or radiation damage. Electrical contacts can be disrupted by pressure cycling. Careful experimental design and data correction are required.
